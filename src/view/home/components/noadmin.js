...
import { Carousel, Col, Row, Card, Avatar, Tooltip, Tag, DatePicker } from 'antd';
import { EditOutlined, GithubOutlined, createFromIconfontCN, Html5Outlined } from '@ant-design/icons';
...
export default function Index() {
	...
	//当地的天气
	const getLocaltion = () => {
		axios.get('https://restapi.amap.com/v3/ip?key=c336900aba752fc9549a670d19a93d4f').then(({ data }) => {
			if (data.status === '1') {
				axios
					.get(
						`https://restapi.amap.com/v3/weather/weatherInfo?city=${data.adcode}&key=c336900aba752fc9549a670d19a93d4f`
					)
					.then(({ data }) => {
						if (data.status === '1') {
				...
				<Row>
						...
						<Card style={{ width: '95%', margin: '20px auto 0' }}>
							<h3>
								...
								{`${new Date().getFullYear()}年${
									new Date().getMonth() + 1
								}月${new Date().getDate()}日,属于${getConstellation(new Date().getMonth() + 1, new Date().getDate())}`}
							</h3>
							<div>
					...
					<Col xs={24} sm={24} md={11} lg={8} xl={8} className='right'>
						...
						<Card style={{ width: 300, margin: '20px auto 0' }}>
							<Tag color='#198f27'>javascript</Tag>
							<Tag color='#2db7f5'>css</Tag>
							<Tag color='#edac5a' icon={<Html5Outlined />}>
								html5
							</Tag>
							<Tag color='#2762b9'>
						...
						<Card style={{ width: 300, margin: '20px auto 0' }}>
							...
							<h3>{opendate}</h3>
							{location.province}
							{location.city}，{location.weather}，当前温度{location.temperature}℃
						</Card>
						<Card style={{ width: 300, margin: '20px auto 0' }}>
							<div>友链</div>
							<a href='https://ch3nnn.cn/' target='_blank' className={style.link}>
								Ch3nnnnn 折腾不息 · 乐此不疲
							</a>
						</Card>
					</Col>
